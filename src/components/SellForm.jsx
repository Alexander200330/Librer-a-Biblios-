...
const SellForm = () => {
    ...
    const handleSubmit = (e) => {
        ...
        const sellObj = new SellModel(idBook, Number(unityPrice), Number(totalPrice), dateSell, Number(quantity));


        if(!sellSchema.isValid(sellObj.getJSON()) || Object.values(sellObj).includes('')){
            setErrores('Todos los campos son obligatorios');
            return;
        }

        postSell(sellObj.getJSONToDB()).then(e => {console.log(e)}).catch(e => {console.log(e)})
    ...
    return (
        <form onSubmit={handleSubmit} encType="multipart/form-data">
            ...
            <input className="bg-blue-800 hover:bg-blue-900 w-full mt-5 text-white uppercase font-bold px-3  py-2 rounded-md hover:cursor-pointer" type='submit' value="Crear venta" />
        </form>
    )
}
